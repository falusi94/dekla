...
% megoldase(+SSpec,+SSol) : sikeres, ha az SSol ÃƒÂ©rtÃƒÂ©k-mÃƒÂ¡trix megoldÃƒÂ¡sa az SSpec Sudoku-feladvÃƒÂ¡nynak.
megoldase(s(PARAM, MATRIX), SOLUTION) :-
    %print([PARAM, MATRIX, SOLUTION]),
    and(
...
% Check every square has all number from 1 to PARAM*PARAM
...
%checkRowIntegrity([[]|_],[[]|_], _, _, _).

% Check if 'west' is true, the sum of number and the one on the left is odd
checkWest(_, false, _, _, _).
checkWest(_, true, ROW, COL, SOLUTION) :-
    COL1 is COL-1,
    getItem(SOLUTION, ROW, COL1, VALUE1),
    getItem(SOLUTION, ROW, COL, VALUE),
    SUM is VALUE1+VALUE,
    mod(SUM, 2, RET),
    RET=:=1.
...
checkSouth(_, true, ROW, COL, SOLUTION) :-
    ROW1 is ROW+1,
    getItem(SOLUTION, ROW1, COL, VALUE1),
    getItem(SOLUTION, ROW, COL, VALUE),
    SUM is VALUE1+VALUE,
    mod(SUM, 2, RET),
    RET=:=1.
